...
const Login = () => {
  ...
  return (
    ...
    viewport={{once:true}}
        ...
        className='relative bg-white p-10 rounded-xl text-slate-500 flex flex-col items-center'>
            <h1 className='text-center text-2xl text-neutral-700 font-medium'>{state}</h1>
            {state === 'Login' && <p className='text-sm'>Welcome back ! PLease sign in to continue .</p>}
            {state !== 'Login' && <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5' >
                <img src={assets.profile_icon} alt="" width={20} height={10} />
            ...
            <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5' >
                ...
                <input onChange={e =>setEmail(e.target.value)} value={email} type="email" placeholder='Email Address' required className='outline-none text-sm' />


            </div><div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5' >
